...
export default function State() {
    ...
    const clickEvent = useCallback(()=>{
      console.log('callBack 테스트 1');
      numberTest += numberTest;
      setNumber(number + 1);
    },[number])

    const clickEvent2 = () =>{
      console.log('callBack 테스트 2');
      numberTest += numberTest;
      setNumber(number + 1);
    }

    useEffect(()=>{
      console.log('callBack 변경 1');
    },[clickEvent])

    useEffect(()=>{
      console.log('callBack 변경 2');
    },[clickEvent2])
    ...
    return (
      <div className="App">
        ...
        <button onClick={clickEvent2}>증가2</button>
        ...
        <button onClick={()=>setToggle(!toggle)}>{toggle.toString()} callback</button>
      </div>
    )
}
